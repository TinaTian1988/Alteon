...
namespace TcpServer
{
    ...
    public sealed class Service
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public static Api_ClientEquipment IsExistEquipment(int id)
        {
            return db.SqlQuery<Api_ClientEquipment>(@"select * from Api_ClientEquipment where Id=@Id", new { @Id = id }).FirstOrDefault();
        }
        /// <summary>
        /// 根据设备Id和数据标识查找数据
        /// </summary>
        ...
        {
            return db.SqlQuery<int>("select Id from Api_ClientEquipment where GateNo=@GateNo and Owner_Id=@Owner_Id and IsDelete=0", new { @GateNo = gateNo, @Owner_Id = ownerId }).FirstOrDefault();
        }
        ...
        public static void UploadData(int equipmentId, List<EquipmentData> dataList)
        {
            ...
            for (int i = 0; i < dataList.Count; i++)
            {
                ...
                if (model == null)
                {
                    //continue;
                    //如果是不存在的标识则添加
                    db.ExecuteSqlCommand("insert Api_EquipmentData (EquipmentId,Mark,SortingIndex,IsDelete) values (@EquipmentId,@Mark,0,0)", new { @EquipmentId = equipmentId, @Mark = dataList[i].Name });
                    model = IsExistEquimentData(equipmentId, dataList[i].Name);
                }
