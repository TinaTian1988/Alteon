...
namespace Alteon.Core.Data
{
    public class Validator
    {
        ...
        /// <param name="strln"></param>
        /// <returns></returns>
        public static bool IsGuid(string strln)
        {
            if (strln != null)
            {
                Regex guidRegEx = new Regex(@"^(\{{0,1}([0-9a-zA-Z]){8}-([0-9a-zA-F]){4}-([0-9a-zA-Z]){4}-([0-9a-zA-Z]){4}-([0-9a-zA-Z]){12}\}{0,1})$");
                return guidRegEx.IsMatch(strln);
            }
            return false;
        }
